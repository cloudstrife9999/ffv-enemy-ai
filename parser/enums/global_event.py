...
class GlobalEvent(Enum):
    ...
    GILGAMESH_AVAILABLE_IN_THE_INTERDIMENSIONAL_RIFT = (GlobalEventTable.FIRST, 0x02)
    KING_BEHEMOTH_DEFEATED = (GlobalEventTable.FIRST, 0x04)
    ...
    CHARACTER_PRESENT_IN_TOP_PARTY_SLOT = (GlobalEventTable.THIRD, 0x80)
    CHARACTER_PRESENT_IN_SECOND_PARTY_SLOT = (GlobalEventTable.THIRD, 0x40)
    CHARACTER_PRESENT_IN_THIRD_PARTY_SLOT = (GlobalEventTable.THIRD, 0x20)
    CHARACTER_PRESENT_IN_FOURTH_PARTY_SLOT = (GlobalEventTable.THIRD, 0x10)
    UNKNOWN_0308_FLAG = (GlobalEventTable.THIRD, 0x08)
    ...
    @classmethod
    def from_table_and_mask(cls, table: GlobalEventTable, mask: int) -> "GlobalEvent":
        for status in cls:
