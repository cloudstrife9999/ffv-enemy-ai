...
class SNESToggleStatusAction(AIRuleAction):
    def __init__(self, property_table: SNESStatsAndPropertiesTable, mask: int) -> None:
    ...
    @property
    def property_table(self) -> SNESStatsAndPropertiesTable:
        if self.raw_second_byte is None:
            raise ValueError("Party member offset is not set.")
        else:
            return SNESStatsAndPropertiesTable(self.raw_second_byte)
    ...
    def to_json(self) -> str | dict[str, Any]:
        if self.property_table == SNESStatsAndPropertiesTable.STATUS_1:
