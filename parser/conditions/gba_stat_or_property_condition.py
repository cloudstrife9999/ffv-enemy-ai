...
class GBAStatOrPropertyCondition(AIRuleCondition):
    def __init__(self, target: Target, property_table: GBAStatsAndPropertiesTable, expected_value: int) -> None:
    ...
    @property
    def stats_and_properties_table(self) -> GBAStatsAndPropertiesTable:
        return GBAStatsAndPropertiesTable(self.raw_third_byte)
    ...
    def to_json(self) -> str | dict[str, Any]:
        ...
        } if self.stats_and_properties_table != GBAStatsAndPropertiesTable.ACTION_FLAGS else {
            "condition_code": self.condition_code.name,
            "target": self.target.name,
            "stats_and_properties_table": self.stats_and_properties_table.name,
    ...
    def to_compact_representation(self, indent: int) -> list[str]:
        if self.stats_and_properties_table == GBAStatsAndPropertiesTable.ACTION_FLAGS:
